...
def fetch_by_user_names(user_name_file):
    ...
    list_of_breaking_bad_actors =[]
    for line in sn_file:
        list_of_breaking_bad_actors.append(line)
    writer = csv.writer(sys.stdout)
    print("user_name" + "," + "tweet")
    for each_actor in list_of_breaking_bad_actors:
        each_actor.strip()
        parameters = [("screen_name",each_actor),("count",100)]
        response = twitterreq(url, "GET", parameters)
        ...
        for each_tweet in json_obj:
            each_tweet['text'] = each_tweet['text'].replace('\n','')
            
            writer.writerow([each_actor.strip(),each_tweet['text']])
